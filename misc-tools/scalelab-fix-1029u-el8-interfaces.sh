...
if ip a show ens2f0 1>/dev/null 2>&1 ; then
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-ens2f0.103 <<EOF
DEVICE=ens2f0.103
NAME=ens2f0.103
VLAN=yes
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-ens2f0.200 <<EOF
DEVICE=ens2f0.200
NAME=ens2f0.200
VLAN=yes
BOOTPROTO=static
...
if ip a show ens2f1 1>/dev/null 2>&1 ; then
cat > /etc/sysconfig/network-scripts/ifcfg-ens2f1 <<EOF
DEVICE=ens2f1
NAME=ens2f1
TYPE=Ethernet
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-ens2f1.104 <<EOF
DEVICE=ens2f1.104
NAME=ens2f1.104
VLAN=yes
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-ens2f1.200 <<EOF
DEVICE=ens2f1.200
NAME=ens2f1.200
VLAN=yes
BOOTPROTO=static
...
for i in enp175s0f0 enp175s0f0.101 enp175s0f0.200 enp175s0f1 enp175s0f1.102 enp175s0f1.200 ens2f0 ens2f0.103 ens2f0.200 ens2f1 ens2f1.104 ens2f1.200 ; do ifup $i ; done
