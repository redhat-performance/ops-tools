...
if ip a show enp1s0f0 1>/dev/null 2>&1 ; then
cat > /etc/sysconfig/network-scripts/ifcfg-enp1s0f0 <<EOF
DEVICE=enp1s0f0
NAME=enp1s0f0
TYPE=Ethernet
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-enp1s0f0.101 <<EOF
DEVICE=enp1s0f0.101
NAME=enp1s0f0.101
VLAN=yes
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-enp1s0f0.200 <<EOF
DEVICE=enp1s0f0.200
NAME=enp1s0f0.200
VLAN=yes
BOOTPROTO=static
...
if ip a show enp1s0f1 1>/dev/null 2>&1 ; then
cat > /etc/sysconfig/network-scripts/ifcfg-enp1s0f1 <<EOF
DEVICE=enp1s0f1
NAME=enp1s0f1
TYPE=Ethernet
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-enp1s0f1.102 <<EOF
DEVICE=enp1s0f1.102
NAME=enp1s0f1.102
VLAN=yes
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-enp1s0f1.200 <<EOF
DEVICE=enp1s0f1.200
NAME=enp1s0f1.200
VLAN=yes
BOOTPROTO=static
...
if ip a show enp2s0f1 1>/dev/null 2>&1 ; then
cat > /etc/sysconfig/network-scripts/ifcfg-enp2s0f1 <<EOF
DEVICE=enp2s0f1
NAME=enp2s0f1
TYPE=Ethernet
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-enp2s0f1.103 <<EOF
DEVICE=enp2s0f1.103
NAME=enp2s0f1.103
VLAN=yes
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-enp2s0f1.200 <<EOF
DEVICE=enp2s0f1.200
NAME=enp2s0f1.200
VLAN=yes
BOOTPROTO=static
...
NETMASK=255.255.0.0
EOF
cat > /etc/sysconfig/network-scripts/ifcfg-enp1s0f1.104 <<EOF
DEVICE=enp1s0f1.104
NAME=enp1s0f1.104
VLAN=yes
BOOTPROTO=static
...
for i in enp1s0f0 enp1s0f0.101 enp1s0f0.200 enp1s0f1 enp1s0f1.102 enp1s0f1.200 enp2s0f1 enp2s0f1.103 enp2s0f1.200 enp1s0f1 enp1s0f1.104 enp1s0f1.200 ; do ifup $i ; done
